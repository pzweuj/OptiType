...
def is_frequent(allele_id: str) -> bool:
    allele_id = allele_id.split('_')[0]
    allele_info = table.loc[allele_id]
    return (allele_info['4digit'] in freq_alleles and allele_info['flags'] == 0) or (allele_info['locus'] in 'HGJ')


def get_4digit(allele_id: str) -> str:
    allele_id = allele_id.split('_')[0]
    return table.loc[allele_id]['4digit']


def get_types(allele_id: str) -> str:
    if isinstance(allele_id, str):
...
def get_num_threads(configured_threads: int) -> int:
    try:
        ...
        return min(multiprocessing.cpu_count(), configured_threads)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--deletebam', 
                        type=bool, 
                        default=True, 
                        help="Delete intermediate bam files after OptiType finished loading them.")
    parser.add_argument('--unpaired_weight', 
                        type=float, 
                        default=0, 
                        help="Weight for unpaired reads.")
    parser.add_argument('--use_discordant', 
                        type=bool, 
                        default=False, 
                        help="Use discordant read pairs.")

    args = parser.parse_args()

    unpaired_weight = args.unpaired_weight
    ...
    if args.beta < 0.0 or args.beta >= 0.1:
        logger.error("Beta value is not correctly chosen. Please choose another beta value between [0,0.1]")
        sys.exit(-1)

    if args.enumerate <= 0:
        print("The specified number of enumerations must be bigger than 0")  # 简化条件逻辑
        sys.exit(-1)
    ...
    bam_input = input_extension in ('sam', 'bam', 'SAM', 'BAM')  # 简化条件逻辑
    ...
    MAPPING_CMD = args.razers3 + " " + COMMAND
    ...
    if not bam_input:
        threads = get_num_threads(args.mapping_threads)
        if VERBOSE:
            logger.info(f"Mapping with {threads} threads...")
        for (i, sample), outbam in zip(enumerate(args.input), bam_paths):          
            if VERBOSE:
                logger.info(f"Mapping {os.path.basename(sample)} to {ref_type.upper()} reference...")

            subprocess.run(MAPPING_CMD % (threads, outbam, MAPPING_REF[ref_type], sample), shell=True, check=True)

    # sam-to-hdf5
    table, features = ht.load_hdf(ALLELE_HDF, False, 'table', 'features')
    if VERBOSE:
        logger.info("Generating binary hit matrix.")
    ...
    if is_paired:
        ...
        if not bam_input and args.deletebam:
    ...
    alleles_to_keep = [allele for allele in binary.columns if is_frequent(allele)]  # 使用列表推导
    binary = binary[alleles_to_keep]

    if VERBOSE:
        logger.info("Temporary pruning of identical rows and columns")
    ...
    if VERBOSE:
        logger.info(f"Size of mtx with unique rows and columns: {temp_pruned.shape}")
        logger.info("Determining minimal set of non-overshadowed alleles")

    minimal_alleles = ht.prune_overshadowed_alleles(temp_pruned)

    if VERBOSE:
        logger.info(f"Keeping only the minimal number of required alleles: {minimal_alleles.shape}")

    binary = binary[minimal_alleles]

    if VERBOSE:
        logger.info("Creating compact model...")
    ...
    threads = get_num_threads(args.ilp_threads)
    if VERBOSE:
        logger.info(f"Starting ilp solver with {threads} threads...")
        logger.info("Initializing OptiType model...")

    op = OptiType(sparse_dict, compact_occ, groups_4digit, table, args.beta, 2,
                  args.solver, threads, verbosity=VERBOSE)
    result = op.solve(args.enumerate)

    if VERBOSE:
        logger.info("Result dataframe has been constructed...")

    # result_4digit = result.applymap(get_types)
    result_4digit = result.map(get_types)
    for iii in ["A1", "A2", "B1", "B2", "C1", "C2"]:
        result_4digit[iii] = result_4digit.get(iii, None)
